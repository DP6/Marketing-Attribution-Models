...
class ShapleyModel(BaseModel):
    def __init__(self, max_size: int = 4, value_column: str = "conv_rate", separator: str = " > "):
    ...
    def _calculate_shapley_for_journey(self, journey_channels, conv_dict, order):
        ...
        for mask in range(1, 1 << n):
            ...
            for i in range(n):
                if (mask & (1 << i)) != 0:
                    W[idx, i] = math.factorial(s - 1) * math.factorial(n - s) / math.factorial(n)
                else:
                    W[idx, i] = -math.factorial(s) * math.factorial(n - s - 1) / math.factorial(n)
    ...
    def _compute_shapley(self, df: pl.DataFrame, order: bool = False):
        ...
        conv_table = (
            ...
            .agg([
                (pl.col("has_conversion").cast(pl.Int64) * pl.col("weight")).sum().alias("conversions"),
        ...
        conv_dict = dict(zip(conv_table_filtered["combinations"], conv_table_filtered[self.value_column]))
        
        results = []
        for comb in conv_table_filtered["combinations"]:
            journey_channels = comb.split(self.separator)
            shapley_vals = self._calculate_shapley_for_journey(journey_channels, conv_dict, order)
        ...
        if self.value_column in ("conv_rate", "custom_value"):
            for res, total_seq, conv_val in zip(results, conv_table_filtered["total_sequences"], conv_table_filtered["conversion_value"]):
    ...
    def calculate(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        exploded = df_with_comb.select(["journey_id", "combinations", "channels", "has_conversion", "weight", "sum_conv_weight"]).explode("channels", empty_as_null=True)
        ...
        exploded = exploded.join(
            ...
            pl.col("attribution_val").sum().over("journey_id").alias("journey_sum_val")
        ).with_columns(
            pl.when(pl.col("has_conversion") & (pl.col("journey_sum_val") > 0))
            .then(pl.col("attribution_val") * pl.col("weight").cast(pl.Float64) * (pl.col("has_conversion").cast(pl.Float64) / pl.col("journey_sum_val")))
            .otherwise(0.0)
    ...
    def get_aggregated_results(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        ]).explode(["channels", "weight"], empty_as_null=True)
