...
class DP6tribution:
    def __init__(
        ...
        """
        MAM ( ) is a class inspired on the R Packages ‘GameTheoryAllocation’ from Alejandro Saavedra-Nieves and  
        ‘ChannelAttribution’ from Davide Altomare and David Loris that was created to bring these concepts to
        Python and to help us understand how the different marketing channels behave during the customer journey.

        Parameters:

          df = None by defaut, but should only be None if choosing to use a random dataframe. Otherwise, 
              have to recieve a Pandas dataframe;

          time_till_conv_colname = None by defaut. Column name in the df containing the time in hours untill 
          ...
          conversion_value = 1 by defaut. Integer that represents a monetary value of a 'conversion', can
                              also receive a string indicating the column name on the dataframe containing the 
                              conversion values;

          channels_colname = None by defaut. Column name in the df containing the different channels during the
                            customer journey. The column must have the same elements as the time_till_conv_colname 
                            has.  
                                  
                            Values could be on a list ou a string with a separator;

          journey_with_conv_colname = None by defaut.

          group_channels = False by defaut. Most important parameter on this class. This indicates the input 
          ...
          group_channels_by_id_list = [] by defaut.

          group_timestamp_colname = None by defaut.

          create_journey_id_based_on_conversion = False by defaut.

          path_separator = ' > ' by defaut. If using 'group_channels = True', this should match the separator
                          being used on the inputed dataframe in the channels_colname;

          verbose = False by defaut. Internal parameter for printing whille 
                    working with MAM (Multi Tocci Attribution);

          random_df = False by defaut. Will create a random dataframe with testing purpose; 
    ...
    def as_pd_dataframe(self):
      """
      Return inputed atributtes as a Pandas Data Frame on self.DataFrame
      """
    ...
    def plot(self, 
      ...
      if not isinstance(other_df, pd.DataFrame):
        # Checking if threre are any results on self.group_by_channels_models 
    ...
    def group_by_results_function(self, channels_value, model_name): 
      """
      Internal function to genarate the group_by_channels_models. A pandas DF containing 
    ...
    def attribution_last_click(self, group_by_channels_models=True):
        """
        The last touchpoint recieves all the credit

        Parameters:
        group_by_channels_models= True by defaut. Will aggregate the attributed results by each channel on 
    ...
    def attribution_position_based(
        ...
        """
        First and last contact have pre set values, the middle touchpoints are evenly distibuted with the chosen weight.

        Defaut:
         - First channel = 0.4
         - Distributed between the middle = 0.2
         - Last channel = 0.4
    ...
    def attribution_time_decay(
        ...
        Parameters:
        decay_over_time = percentage that will be lost by time away from the convertion
    ...
    def attribution_markov(self, transition_to_same_state=False, group_by_channels_models=True):
      ...
      def power_to_infinity(matrix):
          """ Eleva uma matriz quadrada ao infinito usando a decomposição de autovetores.
              É importante que as linhas da matriz somem sempre 1.
              https://en.wikipedia.org/wiki/Eigendecomposition_of_a_matrix
              M = Q*L*inv(Q), onde L=diagonal de autovalores, Q=matrix de autovetores
              M^N = Q*(L^N)*inv(Q)
          """
          eigen_value, eigen_vectors = np.linalg.eig(matrix)

          # No infinito tudo converge para 0 ou 1, por isso usamos np.trunc()
    ...
    def coalitions(self, size = 4, unique_channels = None, order=False):
      ...
      **unique_channels** = By defaut will check self.channels unique values, or a list of channels can be passed
      as well.
    ...
    def attribution_shapley(self, size=4, order=False, values_col='conv_rate', group_by_channels_models = True):
      ...
      than the defined limit, the last N channels will be considered.
                Its also important to accentuate that increasing the number of channels, increases the number calculations
                exponentially.

      order = Boolean that indicates if the order of channels matters during the process.

      values_col = The conversion rate is used by defaut, but the other columns in the journey_conversion_table
