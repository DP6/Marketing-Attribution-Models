...
for i in range(num_conversions):
    ...
    path_len = int(np.random.choice(range(1, 16), p=[
        0.15, 0.15, 0.12, 0.10, 0.10, 0.08, 0.07, 0.06, 0.05, 0.04, 0.03, 0.02, 0.015, 0.01, 0.005
    ]))
    ...
    if path_len > 1:
        times_list = sorted([float(np.random.randint(1, 350)) for _ in range(path_len - 1)], reverse=True)
...
for i in range(num_non_conversions):
    path_len = int(np.random.choice(range(1, 11), p=[
        0.20, 0.20, 0.15, 0.15, 0.10, 0.08, 0.05, 0.04, 0.02, 0.01
    ]))
    ...
    times_list = sorted([float(np.random.randint(1, 350)) for _ in range(path_len)], reverse=True)
    times_str = " > ".join([f"{t:.1f}" for t in times_list])
...
for m in ["markov", "first_click", "last_click"]:
    print(f"Model {m} Attributed Sum: {sum(report_data['attribution_results'][m]['attributions'])}")
